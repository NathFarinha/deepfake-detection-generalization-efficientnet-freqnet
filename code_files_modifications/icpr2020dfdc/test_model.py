...
def main():
    ...
    # Make testsets optional now (can run only with --gengan_dir)
    parser.add_argument('--testsets', type=str, help='Testing datasets', nargs='+',
                        choices=split.available_datasets, required=False)
    ...
    # NEW: GENGAN dataset folder (process each subfolder)
    parser.add_argument('--gengan_dir', type=str, default=None,
                        help='Path to the GANGen-Detection root (will process each subfolder like AttGAN, BEGAN, ... ).')
    ...
    parser.add_argument('--results_dir', type=Path, help='Output folder', default='results/')
    ...
    test_sets = args.testsets or []  # now may be empty
    ...
    gengan_dir = Path(args.gengan_dir) if args.gengan_dir is not None else None

    # try to parse face_policy/patch_size/net_name from model_path; fallback if not present
    try:
        ...
        print("WARNING: Couldn't parse face/size/net from model filename. Using fallbacks.")
        face_policy = 'face'
        patch_size = 224
        net_name = 'FeatureExtractor'  # fallback to the default class imported above
    ...
    # Load net class
    if not hasattr(fornet, net_name):
        print(f"WARNING: net '{net_name}' not found in architectures.fornet. Falling back to 'FeatureExtractor'.")
        net_class = FeatureExtractor
    else:
    ...
    if 'net' not in state_tmp.keys():
        # some checkpoints save only the state_dict without a 'net' wrapper
    ...
    # If user provided standard testsets, build splits as before
    if len(test_sets) > 0:
        ...
        # Build extr_list from standard splits
        if 'train' in test_splits:
            for idx, dataset in enumerate(test_sets):
                extr_list.append(
                    (dfs_out_train[idx], Path(results_dir).joinpath(model_name, dataset + '_train.pkl'), train_roots[idx], dataset + ' TRAIN')
                )
        if 'val' in test_splits:
            for idx, dataset in enumerate(test_sets):
                extr_list.append(
                    (dfs_out_val[idx], Path(results_dir).joinpath(model_name, dataset + '_val.pkl'), val_roots[idx], dataset + ' VAL')
                )
        if 'test' in test_splits:
            for idx, dataset in enumerate(test_sets):
                extr_list.append(
                    (dfs_out_test[idx], Path(results_dir).joinpath(model_name, dataset + '_test.pkl'), test_roots[idx], dataset + ' TEST')
                )

    # If gengan_dir provided, add each subfolder (AttGAN, BEGAN, ...) as an extr entry


    if gengan_dir is not None:
        if not gengan_dir.exists():
            raise RuntimeError(f'GENGAN dir does not exist: {gengan_dir}')
        # Define the root directory for all GENGAN subdatasets
        df_root = str(gengan_dir)

        for sub in sorted(gengan_dir.iterdir()):
            if not sub.is_dir():
                continue
            data = []
            for cls_name, label_bool in [('0_real', False), ('1_fake', True)]:
                cls_folder = sub.joinpath(cls_name)
                ...
                for img_path in sorted(cls_folder.iterdir()):
                    ...
                    # Constructs the full relative path
                    # Example: 'AttGAN/0_real/10020.png'
                    rel_path = os.path.join(sub.name, cls_name, img_path.name)
                    data.append({'path': rel_path, 'label': label_bool, 'video': img_path.stem})
            if len(data) == 0:
                print(f'WARNING: no images found in {sub}, skipping.')
                continue
            df_sub = pd.DataFrame(data)
            df_path = Path(results_dir).joinpath(model_name, sub.name + '_gengan_test.pkl')

            tag = sub.name + ' GENGAN'
            extr_list.append((df_sub, df_path, df_root, tag))

            print(f'Added GENGAN subdataset: {sub.name} ({len(df_sub)} frames)')

    if len(extr_list) == 0:
        raise RuntimeError("No datasets to test. Provide --testsets and/or --gengan_dir with valid data.")

    # Prepare output folder
    out_folder = Path(results_dir).joinpath(model_name)
    out_folder.mkdir(mode=0o775, parents=True, exist_ok=True)

    # Now process extr_list (both original splits and gengan-added entries)

    for df, df_path, df_root, tag in extr_list:
        if override or not Path(df_path).exists():
            ...
            # The print lines were removed to avoid polluting the output, but might be useful for debugging.
            ...
            Path(df_path).parent.mkdir(parents=True, exist_ok=True)
